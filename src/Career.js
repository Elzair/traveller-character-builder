...
import MT2ECAREERS from './data/mt2e/careers';
import MT2ESKILLS from './data/mt2e/skills';
...
function CareerMT2E({ career, updateCareer, upp, updateUPP, skills, updateSkills, cascadeSkill, updateCascadeSkill, age, onEnlistment, updateLog }) {
    ...
    let careers = MT2ECAREERS.map(c => <div key={`career-${c.name}-div`} className="MT2ECareer">
        <input type="radio" id={`career-${c.name}`} name="career" value={c.name} /> <label className="MT2ECareerLabel" htmlFor={`career-${c.name}`} >{capitalize(c.name)}</label>
    </div>);
