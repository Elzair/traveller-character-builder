...
function EntrySkillMT2E({ career, upp, skills, updateSkills, onSkillSelection, updateLog }) {
    function selectSkill(ev) {
        ev.preventDefault();
    ...
    // If this is the traveller's first career or they enrolled in military school, give them all service skills at level 0.
    // If neither of those things is true, then select one skill from the career's service skill table to be level 0.
    useEffect(() => {
        let careerName = career[career.length-1].branch;
        if (career.length === 1 || careerName === 'army-academy' || careerName === 'naval-academy' || careerName) {
            const careerData = CECAREERS.filter(c => c.name === career[career.length - 1].branch)[0];
            const serviceSkills = careerData.sst.filter(skill => skill.type === 'SKILL').map(skill => skill.name);
            let newLog = [];
